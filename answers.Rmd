...
Calculate a bootstrapped, 95% confidence interval for the median of the Gamma distribution with shape = 2 and scale = 1.
...
for(i in seq(n)) {
  xi <- rgamma(100, 2, 1)
  medx <- median(xi)
  se <- sd(xi) / sqrt(length(xi))
  x[i,1] <- medx + qnorm(0.025) * se
  x[i,2] <- medx + qnorm(0.975) * se
}
colMeans(x)
```
