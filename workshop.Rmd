...
`Predict` and `ggplot` makes one plot for each predictor.  Predictors not shown in plot are set to constants (continuous:median, categorical:mode).  0.95 pointwise confidence limits for $\hat{E}(y|x)$ are shown; use `conf.int=FALSE` to suppress CLs.
...
```{r}
ggplot(Predict(f, age))
```

```{r}
ggplot(Predict(f, age=3:15))
```

```{r}
ggplot(Predict(f, age=seq(3,16,length=150)))
```
